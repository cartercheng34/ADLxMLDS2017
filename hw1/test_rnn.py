...
path = os.path.join(sys.argv[1] , 'mfcc/')
...
def predict(test_seq , params , model_path , max_len , max_indices , map48_char , ids):
    ...
    with tf.Session() as sess:
        model_saver = tf.train.import_meta_graph('rnn_model/layer3-512.ckpt.meta')
        model_saver.restore(sess , tf.train.latest_checkpoint('rnn_model/'))
        ...
        counter = 0
        #output_file = os.path.join(sys.argv[2] , 'output.csv')
...
model_path = 'model/'
test_path = os.path.join(sys.argv[1] , 'mfcc/')
"""
